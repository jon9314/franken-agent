...
class OllamaService:
        ...
        for server_name, server_config in self.servers.items():
            try:
                async with httpx.AsyncClient(
                    ...
                    logger.info(
                        f"Checking for models on Ollama server '{server_config.name}' at {url_to_check}"
                    )
                    ...
                    logger.info(
                        f"Found {len(models_data)} models on server '{server_config.name}'."
                    )

            except Exception as e:
                logger.error(
                    f"An unexpected error occurred while fetching models from '{server_config.name}': {e}",
                    exc_info=True,
                )
    ...
    async def _get_target_server(
    ...
    ) -> Tuple[Optional[OllamaServer], str]:
        """
        ...
        if "/" in model_identifier:
            ...
            if server_nickname in self.servers:
                logger.info(
                    f"Request for model '{specific_model_name}' on explicitly named server '{server_nickname}'."
                )
                return self.servers[server_nickname], specific_model_name
            else:
                logger.warning(
                    f"Server nickname '{server_nickname}' not found. Searching for model '{specific_model_name}' on all servers."
                )
        ...
        try:
            ...
            async with httpx.AsyncClient(
                ...
                response = await client.post(
                    f"{url_str.rstrip('/')}/api/generate", json=payload
                )
                response.raise_for_status()
                return response.json()
        except httpx.HTTPStatusError as e:
            logger.error(
                f"Ollama server at {server_url} returned status {e.response.status_code}: {e.response.text}"
            )
            return {"error": f"Ollama server error: {e.response.status_code}."}
            ...
            logger.error(
                f"Unexpected error making request to Ollama server at {server_url}: {e}"
            )
    ...
    async def generate(
        ...
        logger.info(
            f"Sending prompt to model '{target_model}' on server '{server_to_use.name}'."
        )

        url_str = str(server_to_use.url)
        if "host.docker.internal" in url_str:
            url_str = url_str.replace("host.docker.internal", _resolve_docker_host())
        ...
        try:
            ...
            logger.error(
                f"An unexpected error occurred during model generation: {e}",
                exc_info=True,
            )
            return {
                "error": "An unexpected error occurred while communicating with the AI model."
            }

    async def generate_json(
        self, prompt: str, model: Optional[str] = None
    ) -> Dict[str, Any]:
        """Send a prompt expecting JSON formatted response from Ollama."""
        if not model:
            return {"error": "No model was selected for generation."}
        ...
        ollama_response_raw = await self._make_ollama_request(
            server_to_use.url, payload
        )
        ...
        try:
            ...
            logger.error(
                f"Failed to decode JSON from Ollama response: {e}. Raw response string: '{response_text_json_string}'"
            )
            return {
                "error": "Ollama returned an invalid JSON string that could not be parsed."
            }
